    ...
    def __init__(self, agent, nb):
        ...
        for i in range(nb):
            with open(dir_path + f"\\{agent}\\{i}\\results_{i}" + '.pickle', 'rb') as file:
                ...
                self.duration.append(np.mean(tmp_dict["duration"]))
...
Sarsa_results = AgentResults(agent="SARSAgent", nb=20)
...
plt.title("Survival Percentage w.r.t. duration treatment time", fontsize=15)
plt.xlabel("Treatment Duration [hours]", color="grey", fontsize=15)
plt.ylabel("Survival Percentage [%]", color="grey", fontsize=15)
...
plt.title("Number of doses w.r.t. duration treatment time", fontsize=13, color="grey")
plt.xlabel("Treatment Duration [hours]", color="grey", fontsize=15)
plt.ylabel("Doses [Gy]", color="grey", fontsize=15)
plt.grid(axis='y', alpha=0.5)
plt.legend(fontsize=18)
...
plt.figure(figsize = (16,12))
plt.scatter(df_Q["duration"], df_Q["TCP"], label = "Q-learning")
plt.scatter(df_sarsa["duration"], df_sarsa["TCP"], label="Sarsa")
plt.scatter(df_exp_sarsa["duration"], df_exp_sarsa["TCP"], label="ExpSarsa")
plt.title("TCP w.r.t. duration treatment time", fontsize=22)
plt.xlabel("Treatment Duration [hours]", color="grey", fontsize=15)
plt.ylabel("TCP [%]", color="grey", fontsize=15)
plt.grid(axis='y', alpha=0.5)
plt.legend(fontsize=18)
plt.savefig("duration_tcp.svg", bbox_inches='tight')
plt.show()
